...
# Calcular tiempo de entrega promedio y puntuación por estado
...
page = sidebar.radio("Select a page:", ["Demand Forecast", "Rating and Delivery Time", "Seller Analysis"])
...
if page == "Demand Forecast":
    ...
    if st.button('Go'):
        ...
        plot_img = f'data:image/png;base64,{plot_base64}'
        st.image(plot_img)
        st.write(f"Root Mean Square Error (RMSE): {rmse}")
        st.write(forecast_comparison)

elif page == "Rating and Delivery Time":
    st.header("Rating and Delivery Time Analysis")
    metric = st.selectbox("Select metric:", ['delivery_time', 'review_score'])
    ...
    metric = st.selectbox("Select metric to filter:", ['revenue_final', 'delivery_time_final', 'avg_rating'])
    state_filter = st.selectbox("Select a customer state", options=merged_df['customer_state_summary'].unique())
    filtered_data = merged_df[merged_df['customer_state_summary'] == state_filter]

    fig = px.scatter(
        filtered_data,
        x='delivery_time_final',
        y='avg_rating',
        size='revenue_final',
        color='revenue_final',
        hover_name='seller_id_summary',
        title='Seller Performance',
        labels={'delivery_time_final': 'Delivery Time (days)', 'avg_rating': 'Average Rating', 'revenue_final': 'Revenue'},
        color_continuous_scale='Viridis'
    )

    if metric == 'revenue_final':
        fig.update_traces(marker=dict(size=filtered_data['revenue_final'], color=filtered_data['revenue_final'], colorscale='Viridis'))
        fig.update_layout(title='Seller Analysis: Revenue', coloraxis_colorbar=dict(title='Revenue'))
    elif metric == 'delivery_time_final':
        fig.update_traces(marker=dict(size=filtered_data['avg_rating'], color=filtered_data['delivery_time_summary'], colorscale='Reds'))
        fig.update_layout(title='Seller Analysis: Delivery Time', coloraxis_colorbar=dict(title='Delivery Time'))
    elif metric == 'avg_rating':
        fig.update_traces(marker=dict(size=filtered_data['avg_rating'], color=filtered_data['avg_rating'], colorscale='Blues'))
        fig.update_layout(title='Seller Analysis: Rating', coloraxis_colorbar=dict(title='Rating'))
